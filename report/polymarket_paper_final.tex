...
Figure~\ref{fig:architecture} shows the full pipeline. The four agents run in sequence, each persisting its output to PostgreSQL before handing off, so that a downstream failure never destroys upstream work.
...
\begin{tikzpicture}[
    ...
    agent/.style={rectangle, rounded corners, draw=blue!60, fill=blue!10,
        text width=2.6cm, minimum height=1.2cm, align=center, font=\small\bfseries},
    data/.style={rectangle, draw=orange!60, fill=orange!10,
        text width=2.2cm, minimum height=0.8cm, align=center, font=\scriptsize},
    db/.style={cylinder, draw=green!60, fill=green!10,
        shape border rotate=90, aspect=0.25, minimum height=1.0cm,
        minimum width=1.6cm, font=\scriptsize\bfseries, align=center},
    arrow/.style={-Stealth, thick, draw=gray!70}
]

% Agents in a row
\node[agent] (dc) {Agent 1:\\Data Collection};
\node[agent, right=1.4cm of dc] (sa) {Agent 2:\\Sentiment Analysis};
\node[agent, right=1.4cm of sa] (gd) {Agent 3:\\Gap Detection};
\node[agent, right=1.4cm of gd] (rp) {Agent 4:\\Reporting};

% Data sources above Agent 1
\node[data, above=0.6cm of dc, xshift=-1.1cm] (poly) {Polymarket\\Gamma API};
\node[data, above=0.6cm of dc, xshift=1.1cm] (social) {8 Social/News\\Sources};

% Database below the middle of the agent row
\node[db, below=0.7cm of sa] (pgdb) {PostgreSQL\\9 Tables};

% Arrows (data flow)
\draw[arrow] (poly) -- (dc);
\draw[arrow] (social) -- (dc);
\draw[arrow] (dc) -- node[above, font=\tiny] {Contracts+Posts} (sa);
\draw[arrow] (sa) -- node[above, font=\tiny] {Ensemble} (gd);
\draw[arrow] (gd) -- node[above, font=\tiny] {Ranked Gaps} (rp);

% Arrows (db persistence)
...
\caption{Four-agent sequential pipeline architecture. Solid arrows indicate data flow between agents; dashed arrows indicate database persistence at each stage. Every cycle pulls from the Polymarket Gamma API and eight social/news sources, then routes through the four agents while writing intermediate state to PostgreSQL.}
...
\begin{equation}
S(c) = 0.30 V(c) + 0.25 \sigma(c) + 0.20 U(c) + 0.10 E(c) + 0.10 L(c) + 0.05 \Delta(c)
\end{equation}
...
\textbf{Rolling Sentiment Windows.} For each contract, sentiment is aggregated over three temporal windows of 6, 12, and 24 hours. Define the window-mean sentiment as
\begin{equation}
\bar{S}_{w}(c, t) = \frac{1}{|P_{w}|} \sum_{p \in P_{w}} S_{\text{ensemble}}(p), \;\; w \in \{6\text{h}, 12\text{h}, 24\text{h}\}
\end{equation}
where $P_{w}$ is the set of posts about contract $c$ within window $w$ ending at time $t$. The corresponding trend within the window is
\begin{equation}
\Delta S_{w} = S_{\text{latest}} - S_{\text{earliest}} \quad \text{(within window)}.
\end{equation}
These windowed averages and deltas feed directly into the information-asymmetry gap detector described in Strategy 2.
...
This is the primary detector. We convert the mean sentiment score into an implied probability and clamp it to a sane range:
\begin{equation}
P_{\text{implied}} = \text{clamp}\left( P_{\text{market}} + \bar{S} \cdot \alpha, \; 0.05, \; 0.95 \right)
\label{eq:implied}
\end{equation}
where $\bar{S}$ is the mean sentiment for the contract and $\alpha = 0.4$ is the scaling factor. We flag contracts where $|P_{\text{implied}} - P_{\text{market}}| \geq 0.04$ (4 percentage points). The choice of $\alpha$ was, frankly, a guess. Proper calibration would require historical data linking sentiment scores to actual outcome probabilities, which we did not have.

\textbf{Strategy 2: Information Asymmetry.}
Has sentiment shifted significantly in the last 3 hours while market odds stayed flat? The detector compares two sub-windows of the 6-hour sentiment series,
\begin{equation}
\Delta S_{\text{recent}} = \bar{S}_{[t-3\text{h},\,t]} - \bar{S}_{[t-6\text{h},\,t-3\text{h}]},
\end{equation}
and flags a gap when $|\Delta S_{\text{recent}}| \geq 0.10$ and $\text{sign}(\Delta S_{\text{recent}}) \neq \text{sign}(\Delta P_{\text{market}})$. The idea is to catch lags in price discovery: sentiment moved, the market did not.
...
\end{equation}
We flag anything with $|z| \geq 1.5$. With only 8 data points in our historical odds table, this detector was essentially blind, a problem we discuss in our results.
...
Every detected gap receives a dynamic confidence score $C \in [0,100]$ computed from gap size, data volume, cross-source agreement, and contract features:
\begin{equation}
C = \big( \underbrace{f_{\text{gap}} \cdot 40}_{\text{gap size}} + \underbrace{f_{\text{vol}} \cdot 25}_{\text{data volume}} + \underbrace{f_{\text{cons}} \cdot 25}_{\text{consistency}} \big) \cdot m_{\text{social}} \cdot m_{\text{features}}
\end{equation}
where $m_{\text{social}} \in \{0.5, 0.75, 1.0\}$ scales by the number of distinct social sources contributing evidence, and $m_{\text{features}}$ adjusts for contract-specific factors (a near-resolution boost of $\times 1.1$, a high-volatility discount of $\times 0.9$). When social data is thin (fewer than 5 posts), the confidence score is automatically down-weighted. We did not want the system generating high-confidence calls based on three Reddit posts.
...
\begin{table}[H]
\centering
\caption{Per-cycle execution history. Cycle 1a failed early on a database schema error; Cycle 1b was a quick re-init. Cycle 2 was the first long run with no detected gaps. Cycle 3 produced all 30.}
\begin{tabular}{@{}cccccc@{}}
\toprule
\textbf{Cycle} & \textbf{Date} & \textbf{Duration} & \textbf{Posts} & \textbf{Sent.} & \textbf{Gaps} \\
\midrule
1a & Mar 19 & 818s & 0 & 100 & 0 \\
1b & Mar 20 & 2.3s & 0 & 100 & 0 \\
2 & Mar 20 & 8{,}364s & 8{,}023 & 100 & 0 \\
3 & Mar 21 & 6{,}917s & 6{,}274 & 30 & 30 \\
\bottomrule
\end{tabular}
\label{tab:cycles}
\end{table}
...
Figure~\ref{fig:scatter} makes the pattern visible. Each point is one of the 30 detected gaps, plotted as market odds (x-axis) versus the sentiment-implied odds the system computed (y-axis). The dashed diagonal is the perfect-efficiency line where sentiment and market agree. Points above the diagonal are bullish-biased. The blue cluster in the bottom-left is the NHL group: market saying ``almost zero,'' sentiment saying ``about 5\%,'' uniformly across 15 different teams. That is not 15 independent inefficiencies. That is one structural signal repeated 15 times.
...
\end{axis}
\end{tikzpicture}
\caption{Scatter plot of market odds vs.\ sentiment-implied odds for all 30 detected gaps. Points above the diagonal indicate bullish sentiment bias; the NHL cluster at near-zero market odds with $\sim$5\% implied odds represents systematic fan optimism, not market inefficiency.}
\label{fig:scatter}
\end{figure*}

\begin{table}[H]
\centering
\caption{Gap distribution by contract category. NHL longshots dominate the count but carry low confidence; entertainment/meme markets carry high confidence but reflect cross-platform irony rather than information.}
\label{tab:gap_category}
\begin{tabular}{@{}lccr@{}}
\toprule
\textbf{Category} & \textbf{Count} & \textbf{Avg Conf.} & \textbf{Avg Edge} \\
\midrule
NHL Stanley Cup (various) & 15 & 53.1 & 4.72\% \\
Political / Geopolitical & 5 & 70.2 & 10.45\% \\
Sports (non-NHL) & 4 & 44.3 & 5.64\% \\
Entertainment / Meme & 4 & 67.0 & 17.29\% \\
Technology & 1 & 50.0 & 4.40\% \\
NBA Rookie of the Year & 1 & 44.0 & 4.90\% \\
\bottomrule
\end{tabular}
\end{table}
...
This is the fundamental limitation of the current system: it generates predictions without tracking resolution, making it impossible to compute Brier scores, calibration curves, or any standard measure of forecast quality. We ran the system for only three days, and most contracts have resolution dates weeks or months in the future. Our confidence scores are therefore unverified hypotheses. We want to be upfront about that rather than presenting the 30 detected gaps as ``findings'' without the asterisk.
...
% ---------------------------------------------------------------------
\subsection{What a Viable System Would Look Like}
% ---------------------------------------------------------------------

It is worth being concrete about how far the current system is from one that could reasonably be expected to find tradable gaps. Table~\ref{tab:gap_analysis} contrasts what we have now with what would be needed.

\begin{table}[H]
\centering
\caption{Gap analysis: current system vs.\ what would be required for viable gap detection in liquid prediction markets.}
\label{tab:gap_analysis}
\begin{tabular}{@{}p{2.0cm}p{2.7cm}p{2.7cm}@{}}
\toprule
...
This is not a list of incremental tweaks. It is a different system. The cycle frequency alone, going from daily to 5-minute, demands a streaming data architecture rather than batch collection, which is a substantial engineering project on top of the analytical work.
